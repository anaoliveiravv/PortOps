...
export default function Alertas() {
  ...
  const SEV: Record<AlertSeverity, SeverityView> = {
    critical: {
      ...
      card: "border-l-destructive/80 border-destructive/20 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(255,247,248,0.94))]",
    ...
    warning: {
      ...
      card: "border-l-warning/80 border-warning/20 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(255,251,244,0.94))]",
    ...
    info: {
      ...
      card: "border-l-info/80 border-info/20 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(246,250,255,0.94))]",
              ...
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {alert.recommendedAction && (
                    <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs text-[#24364d]">
                  ...
                  <div className="mt-3 flex justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-full border border-[#cbd9ea] bg-white/70 px-3 py-1.5 text-[11px] font-mono font-semibold text-[#102a4c] transition-colors hover:bg-white"
                    >
                    ...
                    <Link
                      ...
                      className={cn("inline-flex items-center gap-1.5 rounded-full border bg-white/70 px-3 py-1.5 text-[11px] font-mono font-semibold transition-colors", view.button)}
                    >
...
function AlertModal({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="alert-modal-title">
      <div className="w-full max-w-2xl overflow-hidden rounded-[1.2rem] border border-[#d5e2f1] bg-white shadow-[0_32px_90px_-42px_rgba(15,23,42,0.7)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#d5e2f1] px-5 py-4">
          <div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-[#d5e2f1] text-muted-foreground transition-colors hover:bg-[#f4f8fd] hover:text-foreground"
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div className="rounded-xl border border-[#d5e2f1] bg-[#f8fafc] p-3 text-sm leading-6 text-[#405672]">
            {alert.description}
          </div>

          {alert.recommendedAction && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-[#24364d]">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            {ship && (
              <ShipLink shipId={ship.id} className="inline-flex items-center rounded-full border border-[#cbd9ea] bg-white px-3 py-1.5 text-[11px] font-mono font-semibold text-[#102a4c] no-underline hover:border-[#9fb7d4] hover:bg-[#f4f8fd]">
                {language === "pt" ? "Ver navio" : language === "en" ? "View vessel" : "查看船舶"}
              </ShipLink>
            )}
            <Link to={riskHref} className="inline-flex items-center gap-1.5 rounded-full border border-[#cbd9ea] bg-white px-3 py-1.5 text-[11px] font-mono font-semibold text-[#102a4c] transition-colors hover:border-[#9fb7d4] hover:bg-[#f4f8fd]">
              {language === "pt" ? "Ver risco" : language === "en" ? "View risk" : "查看风险"} <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
        </div>
...
function InfoTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-[#d5e2f1] bg-white p-3">
